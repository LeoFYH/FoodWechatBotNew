...
```text
GET http://127.0.0.1:9000/memory/leo
GET http://127.0.0.1:9000/memory/alice
```
...
```text
DELETE http://127.0.0.1:9000/memory/leo
```
...
```powershell
Invoke-RestMethod -Uri http://127.0.0.1:9000/memory/leo -Method Delete
```
...
同一个客服机器人支持订单入库。订单结构以 `C:\Users\11752\Downloads\接口契约.md` 为准，不再使用旧的 customer/product schema。

三种来源：

- 标准 Excel：解析成 `kind=base/source=excel`，默认已确认，直接写入 SQLite。
- 照片：视觉模型识别成 `kind=base/source=photo` 草稿，微信回复给用户核验；用户发 `确认` 后写入 SQLite。
- 文字加单：AI 识别成 `kind=patch/source=text` 草稿，微信回复给用户核验；用户发 `确认` 后写入 SQLite。

`order_date` 是下单日期/归属日期，Web 工具按它分批。文字里出现 `6.21订` 这类表达时，会优先写入 `order_date=2026-06-21`。`deliver_date` 只是送达/到货备注，不参与归批。

微信命令：

- 发 `订单`：进入订单模式。
- 发 `入库`：进入产成品入库模式。
- 发 `订单：老三家 鸡腿 20件`：直接进入订单模式并整理文字补丁草稿。
- 发 `确认`：确认当前订单草稿并写入 `orders.db`。
- 发 `取消`：清空当前订单草稿。
- 发 `问诊`：切回原来的需求访谈模式。
...
`/api/orders` 只返回 `confirmed=true` 的订单，结构为 Web 工具的馄饨侯格式。`mark_fetched` 在 Web 工具成功并入汇总后调用，重复调用是幂等的。

`/api/receipts` 读取独立的产成品入库库 `receipts.db`，不返回 `store`，不参与订单 `mark_fetched`。
